...
Destructive operational script. Mocked dry-run tests cover paging and date filtering,
but scale, authentication combinations, and deletion behavior require a disposable-share
test before production use. No benchmark establishes a supported object count.

## Verification

Run `./tests/Test-Purge.ps1` for a mocked CLI regression check with no Azure calls.
Always begin with `-WhatIf`. Test directories spanning multiple list pages,
nested paths, cutoff boundaries, and denied deletes on disposable data. Compare
the preview against an independent inventory before removing `-WhatIf`.

## Release and Rollback

There is no hosted service to deploy. Update the reviewed script in the scheduler
or runbook that owns the job. Confirm backups before enabling deletion. Reverting
the script does not restore deleted files; recovery depends on your backup policy.
...
| 🚀 | **Paged listing** – requests up to 5 000 entries/page; validate throughput against your workload |
...
az storage share generate-sas \
  ...
  --permissions dlr --expiry <FUTURE-UTC-EXPIRY> \
